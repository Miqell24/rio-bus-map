...
// Rio de Janeiro: ONE SMTR feed (data.rio) covering every municipal bus the
// city licenses — four consortia (Internorte, Intersul, Santa Cruz,
// Transcarioca) plus MOBI-Rio, which runs the BRT. Everything here rides the
// road graph, so there is a single cfg; the feed uses the EXTENDED route
// types, not the classic ones: 700 = bus, 702 = express bus (the BRT
...
const MODES = [{
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['905']),
  feeds: [
    {
      tag: 'rio', dir: 'data/gtfs', routeTypes: ['700', '702', '200'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
if (tramAll || tramSel.some((l) => /^VLT/.test(l))) MODES.push({
  // The VLT Carioca is a TRAM — street track through the port district — so it
  // takes the family red, not a colour of its own. Official colours in this
  // family are for metro and suburban rail; MetrôRio and SuperVia keep theirs.
...
async function processMode(cfg) {
  ...
  // The amber category is the BRT here: MOBI-Rio's route_type 702 lines run on
  // segregated busways (TransOeste, TransCarioca, TransOlímpica) with their own
  // stations, so they get their own colour and their own toggle instead of
  // disappearing into 440 navy city lines.
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      } else if (r.route_type === '702') {
        // BRT: amber, and a member of the independent metroline category so
        // the frontend can show the busway network on its own
